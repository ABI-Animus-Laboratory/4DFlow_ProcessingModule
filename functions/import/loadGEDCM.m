function [MAG,v,VENC,INFO,filetype,nframes,timeres,res,matrix,slicespace,VoxDims] = loadGEDCM(handles,directory,res,UPSMP)
    ...
    if UPSMP==2
        ...
        for i=1:d
            ...
            VAP2(1:2:end,1:2:end,1:2:end,i)=squeeze(VAP(:,:,:,i)); %replace original data, otherwise interpolation smooths is
        end
        v(:,:,:,1,:)=1.*squeeze(VAP2(:,:,:,:));
        clear VAP VAP2
        set(handles.TextUpdate,'String','Upsampling 25%'); drawnow;
    else
    ...
    if UPSMP==2
        ...
        for i=1:d
            ...
            VLR2(1:2:end,1:2:end,1:2:end,i)=squeeze(VLR(:,:,:,i)); %replace original data, otherwise interpolation smooths is
        end
        v(:,:,:,2,:)=1.*squeeze(VLR2(:,:,:,:));
        clear VLR VLR2
        set(handles.TextUpdate,'String','Upsampling 50%'); drawnow;
    else
        v(:,:,:,2,:)=1.*squeeze(VLR(:,:,:,:));
        clear VLR
    end
    ...
    if UPSMP==2
        ...
        for i=1:d
            ...
            VSI2(1:2:end,1:2:end,1:2:end,i)=squeeze(VSI(:,:,:,i)); %replace original data, otherwise interpolation smooths is
        end
        v(:,:,:,3,:)=1.*squeeze(VSI2(:,:,:,:));
        clear VSI VSI2
        set(handles.TextUpdate,'String','Upsampling 75%'); drawnow;
    else
        v(:,:,:,3,:)=1.*squeeze(VSI(:,:,:,:));
        clear VSI
    end
    ...
    if UPSMP==2
        ...
        MAG2(1:2:end,1:2:end,1:2:end)=MAG; %replace original data, otherwise interpolation smooths is
        MAG=MAG2;
        clear MAG2
        set(handles.TextUpdate,'String','Upsampling 100%'); drawnow;
    end
