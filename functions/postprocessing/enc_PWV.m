function [PWV,R,RawPWV]=enc_PWV(data_struct,PI_scat,time,Labels,params)
    ...
    if params.PWV(3)==1
        ...
        for root=1:3
            ...
            Vals(row,:)=[];
            if ~isempty(Vals(:,1))
                [pwv,r,Raw2,Ex2]=enc_PWV_TTU(Vals,data_struct,time,0,W{root},params);
                RawPWV{2,root}=Raw2;
                ExPWV{2,root}=Ex2;
                PWV(1,root+6) = pwv;
                R(1,root+6) = r;
    ...
    if params.PWV(6)==1
        ...
        for root=1:3
            ...
            Vals(row,:)=[];
            if ~isempty(Vals(:,1))
                [pwv,r,Raw4,Ex4]=enc_PWV_TTU(Vals,data_struct,time,1,[],params);
                RawPWV{4,root}=Raw4;
                ExPWV{4,root}=Ex4;
                PWV(1,root+15) = pwv;
                R(1,root+15) = r;
        ...
        if params.PltFlag==1
            ...
            if params.SaveData==1
                ...
                saveas(gcf,fullfile(path2data,'PWVFit.jpg'))
                close(gcf)
            end    
        end
        if params.SaveData==1
            path2data=string(fullfile(params.data_dir,'derivatives\QVT',params.subject));
            save(fullfile(path2data,'RawPWV.mat'),'RawPWV','ExPWV')
        end    
    end
end
