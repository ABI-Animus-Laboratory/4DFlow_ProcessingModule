function [PWV,R,RawPWV]=enc_PWV(data_struct,PI_scat,time,Labels,params)
    ...
    if params.PWV(1)==1
        ...
        for root=1:3
            ...
            Vals(row,:)=[];
            if ~isempty(Vals(:,1))
                [pwv,A]=enc_PWV_WO(Flows,Vals,time,2,params);
                PWV(1,root) = pwv;
                W{root}=A;
            end
        end
    ...
    if params.PWV(2)==1
        ...
        for root=1:3
            ...
            Vals(row,:)=[];
            if ~isempty(Vals(:,1))
                [pwv,r,Raw1,Ex1]=enc_PWV_XCor(Vals,data_struct,Labels,time,2,W{root},params,root);
                RawPWV{1,root}=Raw1;
                ExPWV{1,root}=Ex1;
                PWV(1,root+3) = pwv;
                %R(1,root+3) = r;
    ...
    if params.PWV(3)==1
        ...
        for root=1:3
            ...
            Vals(row,:)=[];
            if ~isempty(Vals(:,1))
                [pwv,r,Raw2,Ex2]=enc_PWV_TTU(Vals,data_struct,Labels,time,2,W{root},params,root,pwvlim);
                RawPWV{2,root}=Raw2;
                ExPWV{2,root}=Ex2;
                PWV(1,root+6) = pwv;
                %R(1,root+6) = r;
    ...
    if params.PWV(4)==1
        ...
        for root=1:3
            ...
            Vals(row,:)=[];
            if ~isempty(Vals(:,1))
                [pwv,~]=enc_PWV_WO(Flows,Vals,time,0,params);
                PWV(1,root+18) = pwv;
            end
        end
    end
    if params.PWV(5)==1
        ...
        for root=1:3
            ...
            Vals(row,:)=[];
            if ~isempty(Vals(:,1))
                [pwv,r,Raw3,Ex3]=enc_PWV_XCor(Vals,data_struct,Labels,time,0,[],params,root);
                RawPWV{5,root}=Raw3;
                ExPWV{5,root}=Ex3;
                PWV(1,root+21) = pwv;
                %R(1,root+12) = r;
    ...
    if params.PWV(6)==1
        ...
        for root=1:3
            ...
            Vals(row,:)=[];
            if ~isempty(Vals(:,1))
                [pwv,r,Raw4,Ex4]=enc_PWV_TTU(Vals,data_struct,Labels,time,0,[],params,root,pwvlim);
                RawPWV{6,root}=Raw4;
                ExPWV{7,root}=Ex4;
                PWV(1,root+24) = pwv;
                %R(1,root+15) = r;
